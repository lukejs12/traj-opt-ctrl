clc;
clear variables;
syms m1 m2 I2 c2 l2 q1(t) q2(t) g u b2
...
% D = .5*b1*diff(q1, 't')^2 + .5*b2*diff(q2, 't')^2;
Q = 0;
D = 0.5*b1*diff(q2, 't')^2 + 0.5*b2*diff(q2, 't')^2;
U = [u; 0];
sys = deriveEom(sysName, coordVars, L, D, U, Q, true);
% eom = evalLagrangian(L, D, coordVars)
% res = subs(eom{2}, diff(q2, 't', 2), 0)
% coeff = simplify(eom{2} - res)/diff(q2, 't', 2)
% simplify(-res/coeff, 100)
% pretty(ans)
